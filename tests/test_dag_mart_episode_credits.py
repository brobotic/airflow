...
from dags import dag_mart_episode_credits as module
...
def test_table_exists_uses_information_schema(monkeypatch):
    ...
    exists = module._table_exists(hook, "title_episode")

    assert exists is True
    sql, params = hook.get_first_calls[0]
    assert "information_schema.tables" in sql
    assert params == ("title_episode",)


def test_required_source_tables_include_episode_dependencies():
    assert "title_basics" in module.REQUIRED_SOURCE_TABLES
    assert "title_episode" in module.REQUIRED_SOURCE_TABLES
...
def test_dag_task_chain_includes_elasticsearch_export():
    dag = module.dag
    assert dag.dag_id == "mart_episode_credits"
...
def test_dag_uses_source_datasets_as_schedule():
    ...
    assert airflow_datasets.TITLE_EPISODE_DATASET.uri in schedule_dataset_uris
