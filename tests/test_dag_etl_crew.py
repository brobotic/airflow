...
from dags import dag_etl_crew as module
...
def test_extract_and_load_transforms_and_upserts(monkeypatch, tmp_path):
    ...
    tsv_content = "\t".join(["tconst", "directors", "writers"]) + "\n"
    tsv_content += "\t".join(["tt0001", "nm0001,nm0002", "nm0101"]) + "\n"
    tsv_content += "\t".join(["tt0002", r"\N", r"\N"]) + "\n"

    tsv_file = tmp_path / "title.crew.tsv"
...
def test_dag_task_chain():
    ...
    dag = module.dag
    assert dag.dag_id == "movies_crew_etl"
...
def test_extract_task_publishes_title_crew_dataset():
    """Ensure extract_and_load publishes the title_crew dataset event."""
    extract_task = module.dag.get_task("extract_and_load")
    outlet_uris = {dataset.uri for dataset in extract_task.outlets}
    assert airflow_datasets.TITLE_CREW_DATASET.uri in outlet_uris
