...
from dags import dag_etl_names as module
...
def test_extract_and_load_transforms_and_upserts(monkeypatch, tmp_path):
    ...
    tsv_content += "\t".join(
        ["nm0001", "Name A", "1970", r"\N", "actor,producer", "tt0001,tt0002"]
    ) + "\n"
    tsv_content += "\t".join(
        ["nm0002", "Name B", r"\N", r"\N", r"\N", r"\N"]
    ) + "\n"
    tsv_content += "\t".join(
        ["nm0003", "Name C", "19x9", "abcd", "director", "tt0003"]
    ) + "\n"

    tsv_file = tmp_path / "name.basics.tsv"
...
def test_dag_task_chain():
    ...
    dag = module.dag
    assert dag.dag_id == "movies_names_etl"
...
def test_extract_task_publishes_name_basics_dataset():
    """Ensure extract_and_load publishes the name_basics dataset event."""
    extract_task = module.dag.get_task("extract_and_load")
    outlet_uris = {dataset.uri for dataset in extract_task.outlets}
    assert airflow_datasets.NAME_BASICS_DATASET.uri in outlet_uris
