...
from dags import dag_mart_series_people_rollup as module
...
def test_table_exists_uses_information_schema(monkeypatch):
    ...
    exists = module._table_exists(hook, "mart_episode_credits")

    assert exists is True
    sql, params = hook.get_first_calls[0]
    assert "information_schema.tables" in sql
    assert params == ("mart_episode_credits",)


def test_required_source_table_uses_episode_credits_mart():
    assert "mart_episode_credits" in module.REQUIRED_SOURCE_TABLES


def test_dag_task_chain_includes_elasticsearch_export():
    dag = module.dag
    assert dag.dag_id == "mart_series_people_rollup"
...
def test_dag_uses_episode_credits_dataset_as_schedule():
    dag = module.dag
    dataset_condition = dag.timetable.dataset_condition
    schedule_dataset_uris = {uri for uri, _ in dataset_condition.iter_datasets()}

    assert airflow_datasets.MART_EPISODE_CREDITS_DATASET.uri in schedule_dataset_uris
