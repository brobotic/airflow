...
from dags import dag_etl_movies as module
...
def test_extract_and_load_transforms_and_inserts(monkeypatch, tmp_path):
    """Validate TSV rows are transformed into expected Python types and bulk inserted once."""
    tsv_content = "\t".join(
        [
            "tconst",
            "titleType",
    ...
    tsv_content += "\t".join(
        ["tt0001", "movie", "A", "A", "1", "1999", r"\N", "120", "Drama"]
    ) + "\n"
    tsv_content += "\t".join(
        ["tt0002", "short", "B", "B", "0", r"\N", r"\N", r"\N", r"\N"]
    ) + "\n"
    tsv_content += "\t".join(
        ["tt0003", "tvSeries", "C", "C", "0", "20x4", "abcd", "9x", "Comedy"]
    ) + "\n"
...
def test_extract_task_publishes_title_basics_dataset():
    """Ensure extract_and_load publishes the title_basics dataset event."""
    extract_task = module.dag.get_task("extract_and_load")
    outlet_uris = {dataset.uri for dataset in extract_task.outlets}
    assert airflow_datasets.TITLE_BASICS_DATASET.uri in outlet_uris
