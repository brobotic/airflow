...
from dags import dag_mart_episode_enriched as module
...
def test_table_exists_uses_information_schema(monkeypatch):
    ...
    exists = module._table_exists(hook, "title_akas")

    assert exists is True
    sql, params = hook.get_first_calls[0]
    assert "information_schema.tables" in sql
    assert params == ("title_akas",)


def test_required_source_tables_include_episode_and_akas():
...
def test_dag_task_chain_includes_elasticsearch_export():
    dag = module.dag
    assert dag.dag_id == "mart_episode_enriched"
