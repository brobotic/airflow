...
try:
    from airflow_datasets import MART_EPISODE_CREDITS_DATASET
    from etl_tasks import create_standard_etl_tasks
    from notifications import notify_discord_failure
except ModuleNotFoundError:
    from dags.airflow_datasets import MART_EPISODE_CREDITS_DATASET
...
TABLE = "mart_series_people_rollup"
REQUIRED_SOURCE_TABLES = ["mart_episode_credits"]

ES_INDEX = os.getenv("ELASTICSEARCH_SERIES_PEOPLE_ROLLUP_INDEX", TABLE)
...
def create_table():
    ...
    hook.run(
        ...
        CREATE TABLE IF NOT EXISTS {TABLE} (
            ...
            last_season           INTEGER,
            first_episode_number  INTEGER,
            last_episode_number   INTEGER,
            avg_episode_rating    DOUBLE PRECISION,
            total_episode_votes   BIGINT,
...
def extract_and_load():
    ...
    hook.run(
        ...
        SELECT
            ...
            mec.person_name,
            mec.credit_type,
            COUNT(DISTINCT mec.episode_tconst) AS episode_count,
            COUNT(DISTINCT mec.season_number) AS season_count,
            MIN(mec.season_number) AS first_season,
            MAX(mec.season_number) AS last_season,
            MIN(mec.episode_number) AS first_episode_number,
            MAX(mec.episode_number) AS last_episode_number,
            AVG(mec.average_rating) AS avg_episode_rating,
            COALESCE(SUM(mec.num_votes), 0) AS total_episode_votes,
            now() AS last_refreshed_at
        FROM mart_episode_credits mec
        GROUP BY
            mec.series_tconst,
            mec.series_title,
            mec.person_nconst,
    ...
    indexes = [
        ...
        f"CREATE INDEX IF NOT EXISTS idx_{TABLE}_episode_count ON {TABLE} (episode_count DESC);",
    ]
...
def verify_load():
    ...
    sample = hook.get_records(
        f"""
        SELECT series_title, person_name, credit_type, episode_count, season_count
        FROM {TABLE}
        ORDER BY episode_count DESC, season_count DESC
        LIMIT 10;
        """
    )
    for row in sample:
...
def export_to_elasticsearch():
    ...
    def actions():
        conn = hook.get_conn()
        cursor = conn.cursor(name="mart_series_people_rollup_export")
        ...
        try:
            ...
            while True:
                ...
                for row in batch:
                    ...
                    doc_id = f"{series_tconst}::{person_nconst}::{credit_type}"
                    yield {
                        "_index": ES_INDEX,
                        "_id": doc_id,
                        "_source": {
                            ...
                            "avg_episode_rating": avg_episode_rating,
                            "total_episode_votes": total_episode_votes,
                            "last_refreshed_at": (
...
with DAG(
    dag_id="mart_series_people_rollup",
    description="Build series-level people rollup from episode-level credits",
    default_args={
        "on_failure_callback": partial(
            notify_discord_failure,
            title="❌ mart_series_people_rollup task failed",
        )
    },
    start_date=datetime(2025, 1, 1),
    schedule=[MART_EPISODE_CREDITS_DATASET],
    catchup=False,
    ...
    _, _, verify_load_task, notify_task = create_standard_etl_tasks(
        ...
        table=TABLE,
        success_title="✅ mart_series_people_rollup completed",
    )
