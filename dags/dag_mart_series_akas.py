...
try:
    from airflow_datasets import MART_EPISODE_ENRICHED_DATASET, TITLE_AKAS_DATASET
    from etl_tasks import create_standard_etl_tasks
    from notifications import notify_discord_failure
except ModuleNotFoundError:
    from dags.airflow_datasets import MART_EPISODE_ENRICHED_DATASET, TITLE_AKAS_DATASET
...
TABLE = "mart_series_akas"
REQUIRED_SOURCE_TABLES = ["mart_episode_enriched", "title_akas"]

ES_INDEX = os.getenv("ELASTICSEARCH_SERIES_AKAS_INDEX", TABLE)
...
def extract_and_load():
    ...
    hook.run(
        ...
        SELECT
            ...
            a.region,
            a.language,
            COUNT(a.title_id)::INTEGER AS aka_count,
            string_agg(DISTINCT a.title, ' | ' ORDER BY a.title) AS sample_titles,
            now() AS last_refreshed_at
        FROM mart_episode_enriched me
        LEFT JOIN title_akas a ON a.title_id = me.episode_tconst
        GROUP BY
            me.series_tconst,
            me.series_title,
            me.episode_tconst,
    ...
    indexes = [
        ...
        f"CREATE INDEX IF NOT EXISTS idx_{TABLE}_region_language ON {TABLE} (region, language);",
        f"CREATE INDEX IF NOT EXISTS idx_{TABLE}_aka_count ON {TABLE} (aka_count DESC);",
    ]
...
def verify_load():
    ...
    regions = hook.get_first(f"SELECT COUNT(DISTINCT region) FROM {TABLE};")[0]

    logging.info("Total rows: %d", total)
    logging.info("Distinct regions: %d", regions)

    sample = hook.get_records(
        f"""
        SELECT series_title, season_number, episode_number, region, language, aka_count
        FROM {TABLE}
        ORDER BY aka_count DESC NULLS LAST
        LIMIT 10;
        """
    )
    for row in sample:
...
def export_to_elasticsearch():
    ...
    def actions():
        conn = hook.get_conn()
        cursor = conn.cursor(name="mart_series_akas_export")
        ...
        try:
            ...
            while True:
                ...
                for row in batch:
                    ...
                    doc_id = f"{episode_tconst}::{region or 'na'}::{language or 'na'}"
                    yield {
                        "_index": ES_INDEX,
                        "_id": doc_id,
                        "_source": {
...
with DAG(
    dag_id="mart_series_akas",
    description="Build series/episode regional title mart from AKAs",
    default_args={
        "on_failure_callback": partial(
            notify_discord_failure,
            title="❌ mart_series_akas task failed",
        )
    },
    start_date=datetime(2025, 1, 1),
    schedule=[MART_EPISODE_ENRICHED_DATASET, TITLE_AKAS_DATASET],
    catchup=False,
    ...
    _, _, verify_load_task, notify_task = create_standard_etl_tasks(
        ...
        table=TABLE,
        success_title="✅ mart_series_akas completed",
    )
